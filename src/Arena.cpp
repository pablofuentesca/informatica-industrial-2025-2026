...
void Arena::inicializa()
{
    ...
    t = 0.0;
    silbatoSonado = false;
    usarDiagonal = ETSIDI::lanzaMoneda();   // elige al azar el tipo de transicion
    estado = TRANSICION;
    ETSIDI::playMusica("sonidos/aficion.wav", true);  // aficion de fondo
}
...
void Arena::dibuja() const
{
    ...
    if (estado == TRANSICION) {
        double prog = smoothstep(clamp01(t / 1.0));
        if (usarDiagonal)
            dibujaDiagonal(prog);
        else
            dibujaIris(prog);
